 ...
 * holding the deployment's public key (GET /manifest-signing-key) can tell an
 * untouched manifest from an edited one.
 *
 * Optional by design: a deployment without MANIFEST_SIGNING_KEY still exports,
 * it just gets `signature: null` and the weaker guarantee. A malformed key
 * throws rather than silently downgrading to unsigned, because a manifest that
 * is quietly unsigned is the failure signing exists to prevent — index.ts
 * additionally validates the key at boot and exits(1) on misconfiguration.
 *
 * Key material is a raw 32-byte Ed25519 seed, hex-encoded
 * (`openssl rand -hex 32`). Dedicated secret — never reuse
 * DOWNLOAD_SIGNING_SECRET or EULEX_MCP_JWT_SECRET.
 */

// PKCS#8 DER prefix for an Ed25519 private key (RFC 8410). This Node version
// (v25.8) does not accept `format: "raw-seed"` in createPrivateKey, so the
// bare 32-byte seed is wrapped in this fixed envelope instead. Ed25519 seeds
// and public keys are both 32 bytes.
 ...
 */
export const SIGNING_CONTEXT = "eulex-project-manifest-v1";
...
function seedFromEnv(): Buffer | null {
    ...
    if (!/^[0-9a-fA-F]{64}$/.test(raw)) {
        throw new Error(
            "MANIFEST_SIGNING_KEY must be a 32-byte hex seed " +
...
/**
 * Deterministic JSON: object keys sorted, array order kept, no whitespace.
 *
...
export function canonicalize(value: unknown): string {
    ...
    // holds JSON scalars from the database, so this should not arise — but a
    // silently wrong serialisation is the same failure as the non-finite
    // number above, and this function is what every digest rests on.
 ...
 * SIGNING_CONTEXT + NUL + the digest bytes (`crypto.sign` with algorithm
 * null, as Ed25519 requires), so it can be checked with any Ed25519
 * implementation and none of this code:
 *
 *   crypto.verify(null,
 *     Buffer.concat([Buffer.from("eulex-project-manifest-v1\0"),
